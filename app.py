...
# PREDVOLENÉ DÁTA (Použijú sa iba úplne prvýkrát, kým sa nevytvorí súbor)
predvolene_rastliny = {
    "rastlina1": {
...
# --- FUNKCIE PRE NAČÍTANIE A UKLADANIE DO SÚBORU ---
def nacitaj_datu():
...
def update_plant():
    ...
    if rid in system_data["rastliny"] and novy_nazov:
        ...
        uloz_datu(system_data)  # OKAMŽITÝ ZÁPIS NA DISK
        return jsonify({"status": "success"})
    return jsonify({"status": "error"}), 400

# API: Úprava mien služieb
...
# API pre Arduino: Prijímanie dát zo senzora
...
def prijmi_data():
    ...
    if rid in system_data["rastliny"]:
        ...
        uloz_datu(system_data)  # Uložíme aj nové grafy a dáta zo senzora
        return jsonify({"status": "success"})
    return jsonify({"status": "error"}), 404

# API pre Web: Posiela kompletné info
@app.route('/api/history/<rastlina_id>')
def daj_historiu(rastlina_id):
    if rastlina_id in system_data["rastliny"]:
        return jsonify({
            "aktualne": system_data["rastliny"][rastlina_id]["aktualne"], 
            "historia": system_data["rastliny"][rastlina_id]["historia"],
            "manualne_poliate": system_data["rastliny"][rastlina_id]["manualne_poliate"]
        })
    return jsonify({"error": "Nenájdené"}), 404

# API pre Tlačidlo: Zapíše čas stlačenia
...
def zaliate(rastlina_id):
    if rastlina_id in system_data["rastliny"]:
        ...
        uloz_datu(system_data)  # Uložíme kliknutie na disk
        return jsonify({"status": "success"})
    return jsonify({"status": "error"}), 404
